...
class MongoDBNamespaceManager(NamespaceManager):
    ...
    def __init__(self, namespace, url=None, data_dir=None,
        ...
        # Temporarily uses a local copy of the functions until pymongo
        # upgrades to new parser code
        (host_list, database, username,
            password, collection, options) = _parse_uri(url)
        ...
        def _create_mongo_conn():
            ...
            conn = Connection(
                host_uri,
                slave_okay=options.get('slaveok', False))
